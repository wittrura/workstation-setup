...
restore_default_bash_it_settings() {
  ...
  if [ -d "$BASH_IT/plugins/enabled" ]; then
    rm -r "${BASH_IT}/plugins/enabled"
  fi
  if [ -d "$BASH_IT/aliases/enabled" ]; then
    rm -r "${BASH_IT}/aliases/enabled"
  fi
  if [ -d "$BASH_IT/completions/enabled" ]; then
    rm -r "${BASH_IT}/completions/enabled"
  fi
}

enable_custom_bash_it_plugin() {
  local filepath=$1
  local filename
